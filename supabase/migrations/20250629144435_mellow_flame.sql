/*
  ...
  This migration cleans up the database for production use by:
  1. Removing unused tables and policies
  2. Ensuring consistent RLS policies
  3. Cleaning up duplicate or test data
  4. Optimizing indexes and constraints
*/
...
CREATE POLICY IF NOT EXISTS "Users can read own google tokens"
  ON google_tokens
  FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own google tokens"
  ON google_tokens
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own google tokens"
  ON google_tokens
  FOR UPDATE
  TO authenticated
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own google tokens"
  ON google_tokens
  FOR DELETE
  TO authenticated
  USING (auth.uid() = user_id);
